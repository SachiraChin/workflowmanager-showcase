...
**Verified:** TUI and WebUI have no direct Python imports from `server/`.

### TUI Contract Usage (Correct Pattern)
All TUI files import shared types from `contracts/` only:
- `tui/api_handler.py` → `from contracts import ...`
- `tui/workflow_runner.py` → `from contracts import ...`
- `tui/strategies/*.py` → `from contracts import InteractionRequest, InteractionResponse`

### Server Contract Usage (Correct Pattern)
Server imports from `contracts/` for shared types:
- `server/api/models.py` → `from contracts.interactions import ...`
- `server/engine/module_interface.py` → `from contracts import ...`
- `server/modules/io/validate.py` → `from contracts.validation import ...`

### Validation Tooling
Available in `/analysis/`:
- `check_tui_deps.py` - Validates TUI doesn't import server modules
- `analyzer.py` - Full codebase dependency analysis

**Conclusion:** No cross-module violations found. Safe to proceed with server restructure without impacting TUI/WebUI.

---

## Design Decisions (from r1 Review)

1. **No backward-compatible import aliases needed** - Only API endpoints matter; internal restructure is invisible to clients

2. **Implementation order approved** - Proceed with proposed 5-step plan

3. **Success criteria** - All API endpoints must work exactly as they do now after restructure

4. **Testing strategy** - Manual validation at each step (no time for comprehensive test suite upfront)

5. **Cross-module isolation confirmed** - TUI/WebUI only use `contracts/` and HTTP API; server restructure is internal
...
| Risk | Mitigation |
|------|------------|
| Breaking existing functionality | Incremental changes with manual testing at each step |
