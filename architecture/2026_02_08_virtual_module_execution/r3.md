...
The implementation has been significantly simplified from what was originally 
specified above. Key changes:

### New Approach: Use Existing Processor

Instead of implementing custom module execution logic in virtual.py, we now use
the existing `WorkflowProcessor` with `VirtualDatabase` passed as the database.
This achieves true 1:1 parity with real workflow execution.

### New Components Added

1. **`ExecutionTarget` dataclass** (`backend/server/models/execution.py`)
   - `step_id: str` - Target step ID
   - `module_name: str` - Target module name
   - Used to specify where to stop execution

2. **`TARGET_REACHED` status** (`backend/server/models/workflow.py`)
   - New `WorkflowStatus` enum value
   - Returned when executor reaches target module (non-interactive)

3. **`target` parameter added to:**
   - `WorkflowExecutor.execute_from_position()`
   - `WorkflowExecutor.execute_step_modules()`
   - `WorkflowExecutor.execute_from_module()`
   - `InteractionHandler.continue_after_interaction()`
   - `WorkflowProcessor.start_workflow()`
   - `WorkflowProcessor.respond()`

### API Changes

**`/respond` request now includes:**
- `virtual_run_id: str` - Required, from start response (reuses same run ID)

**Both responses now include in `result`:**
- `virtual_run_id: str` - The workflow run ID to use in subsequent requests

### Strikethrough: Old Implementation

~~The following sections are superseded by the new implementation:~~

~~### API Routes (lines 447-926)~~

~~The old implementation called modules directly via `module.get_interaction_request()`
and `module.execute_with_response()`. This bypassed the normal executor flow and
required duplicating logic for input resolution, context creation, etc.~~

~~### Helper Functions~~

~~- ~~`find_step_and_module()` - No longer needed, processor handles this~~
~~- ~~`create_virtual_context()` - No longer needed, executor creates context~~
~~- ~~`build_error_response()` - Simplified, processor returns proper errors~~
~~- ~~`get_registry()` - Processor has its own registry~~

### New Implementation

```python
# backend/server/api/routes/virtual.py (simplified)

@router.post("/start", response_model=WorkflowResponse)
async def start_virtual_module(request: VirtualStartRequest, ...):
    vdb = VirtualDatabase(request.virtual_db)
    
    # Store workflow as version (same as real /start)
    ...
    # Use processor (same as real /start)
    ...
    return add_virtual_state_to_result(result, vdb, result.workflow_run_id)


@router.post("/respond", response_model=WorkflowResponse)
async def respond_virtual_module(request: VirtualRespondRequest, ...):
    vdb = VirtualDatabase(request.virtual_db)
    ...
    vdb.workflow_runs.update_one(
        {"workflow_run_id": request.virtual_run_id},
    ...
    # Use processor (same as real /respond)
    processor = WorkflowProcessor(vdb)
    target = ExecutionTarget(
        step_id=request.target_step_id,
        module_name=request.target_module_name,
    ...
    return add_virtual_state_to_result(result, vdb, request.virtual_run_id)
```

### Benefits of New Approach

1. **True 1:1 parity** - Virtual uses exact same code paths as real workflows
2. **Less code** - Reduced from ~470 lines to ~250 lines
3. **Automatic event storage** - Executor stores events to vdb automatically
4. **Jinja resolution works** - State is properly maintained across modules
5. **Workflow updates supported** - Editor can modify workflow between 
   interactions
6. **Version tracking** - Version history is tracked same as real workflows

---

## UI/Editor Integration Plan

The virtual execution system is designed to integrate with the editor 
application for live preview functionality.

### Core Concepts

1. **Separate from Editor Logic** - Virtual execution runtime is a separate 
   process from editor UI. Editor provides workflow data, runtime handles 
   execution. Runtime can be integrated into editor but is not part of it.

2. **Live Preview with Jinja Resolution** - Primary use case is showing real 
   previews instead of static placeholders. Jinja expressions like 
   `{{ state.pet_type_selection }}` resolve using actual state from virtual 
   execution.

3. **Run Up To Target** - When UI wants to preview a module, it provides the 
   target step/module AND all user selections up to that point. The runtime 
   maintains its own state and is responsible for calling `/start` and 
   `/respond` as needed to reach the target. UI does not call these APIs 
   directly - runtime handles the orchestration.

4. **Runtime Maintains Context** - Runtime keeps track of:
   - Current virtual_db state at each module boundary
   - Which modules have been executed
   - Per-module workflow snapshots (to detect changes)
   
   This allows efficient resumption. For example, if UI wants module 10 but 
   only edited module 7, runtime knows nothing changed up to module 6, so it 
   resumes from module 7 using virtual_db state from end of module 6.

5. **Navigation-Aware State** - When user navigates to a module (e.g., clicks 
   module 2 in step 1), runtime provides state as of that module boundary. 
   Going backward shows historical state, not current full state.

6. **Auto-Selection for Prerequisites** - For steps with selections before 
   target, runtime can auto-select values (first option, random, etc.) to 
   allow reaching target without manual interaction for every prerequisite.

### Editor Integration Flow

```
Editor UI                       Virtual Runtime (client-side)
  │                                   │
  │  User clicks on module 5          │
  │  Provides selections for 1-4      │
  │─────────────────────────────────>│
  │                                   │
  │                                   │  Check: anything changed since last run?
  │                                   │  - Modules 1-3: no changes, have cached state
  │                                   │  - Module 4: workflow edited
  │                                   │
  │                                   │  Resume from module 4 checkpoint
  │                                   │  POST /virtual/start (target: module 5)
  │                                   │  virtual_db: state after module 3
  │                                   │
  │                                   │  Backend executes module 4
  │                                   │  Module 4 needs interaction
  │                                   │  Returns AWAITING_INPUT
  │                                   │
  │                                   │  Runtime has selection for module 4
  │                                   │  POST /virtual/respond
  │                                   │  response: provided selection
  │                                   │
  │                                   │  Backend continues to module 5
  │                                   │  Module 5 needs interaction
  │                                   │  Returns AWAITING_INPUT
  │                                   │
  │  Receive: interaction_request     │
  │  for module 5 with resolved Jinja │
  │<─────────────────────────────────│
  │                                   │
  │  Show preview for module 5        │
  │                                   │
```

**Key Points:**
- UI provides target module AND all selections up to that point
- Runtime orchestrates API calls internally
- Runtime maintains checkpoints (virtual_db snapshots per module)
- Runtime detects workflow changes to minimize re-execution
- UI only receives final result for target module

### State Management

- **Fresh Start**: Each session starts fresh (no persistent virtual_db)
- **Session State**: virtual_db maintains state within a session
- **Per-Module Checkpoints**: Runtime saves virtual_db snapshot after each 
  module execution, keyed by module position (stepIndex/moduleIndex)
- **Per-Module Workflow Snapshots**: Runtime tracks workflow JSON per module 
  to detect what changed between runs
- **Efficient Resumption**: When target changes or workflow is edited, runtime 
  finds nearest valid checkpoint and resumes from there
- **Database Adapter**: Keep persistence boundary open for later DB support

### InteractionHost Integration

- Runtime provides all params needed for `InteractionHost` component
- Runtime does NOT render interactions - that's UI's responsibility
- `interaction_request` in response contains everything InteractionHost needs
- UI can render inline in editor sidebar/modal or dedicated panel

### Workflow Updates Between Interactions

Editor can modify workflow between interactions (e.g., user edits module 
config while at an interaction). The `/respond` endpoint:

1. Stores updated workflow as new version in virtual DB
2. Updates workflow run to use new version
3. Tracks version history
4. Continues execution with updated workflow

This enables live editing while previewing.
