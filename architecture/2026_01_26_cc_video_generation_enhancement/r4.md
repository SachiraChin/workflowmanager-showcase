...
6. text_overlays       ← was step 5
7. titles_social       ← was step 6
8. music               ← was step 7
9. workflow_summary    ← was step 8
```

### Rationale

Image analysis generates:
- Environment details (used by video prompts, text overlays)
- Motion elements (used by video generation)
- **Audio atmosphere** (used by music step)

Since the audio data is used by the music step (step 8 → now step 8), image
analysis belongs as its own step rather than inside video generation.
...
| Item | Decision |
|------|----------|
| Image Analysis | New step 4 (separate from video generation) |
  ...
  "modules": [
    ...
    },
    {
      ...
      "inputs": {
        ...
        "data": "{{ state.image_analysis }}",
        "schema": {
          "$ref": "schemas/cc_image_analysis_display_schema.json",
          "type": "json"
        },
        "prompt": "Review image analysis",
        "mode": "review"
      },
      "outputs_to_state": {},
      "retryable": {
        ...
        "options": [
          ...
          },
          {
            "id": "retry",
            "mode": "retry",
            "label": "Re-analyze image",
            "shortcut": "r",
            "target_module": "analyze_image",
            "feedback": {
              "enabled": true,
              "prompt": "What should be different?",
              "default_message": "Please re-analyze with different focus."
            }
          },
          {
            "id": "change_image",
...
#### cc_image_analysis_display_schema.json

<!--why do we need display schema for this, we are not displaying anything this
step, its data will be displayed in other steps. -->
...
```json
{
  ...
  "properties": {
    "environment": {
      ...
      "properties": {
        "setting": {
          "type": "string",
          "_ux.display": true,
          "_ux.display_label": "Setting"
        },
        "time_of_day": {
          "type": "string",
          "_ux.display": true,
          "_ux.display_label": "Time of Day"
        },
        "atmosphere": {
          "type": "string",
          "_ux.display": true,
          "_ux.display_label": "Atmosphere"
        },
        "lighting": {
          "type": "string",
          "_ux.display": true,
        ...
        "key_subjects": {
          "type": "array",
          "_ux.display": true,
          "_ux.display_label": "Key Subjects",
          "_ux.render_as": "tags"
        },
        "dominant_colors": {
          "type": "array",
          "_ux.display": true,
          "_ux.display_label": "Colors",
          "_ux.render_as": "tags"
        }
      }
    },
    "motion_elements": {
      ...
      "properties": {
        "detected_elements": {
          ...
          "_ux": {
            "display": "visible",
            "display_label": "Detected Elements",
            "render_as": "card-list"
          },
          "items": {
            "type": "object",
            "_ux": {
              "render_as": "inline-card"
            },
            "properties": {
              "element": {
                "type": "string",
    ...
    "audio_atmosphere": {
      ...
      "_ux": {
        "display": "visible",
        "display_label": "Audio Atmosphere",
        "render_as": "section"
      },
      "properties": {
        "implied_sounds": {
          "type": "array",
          "_ux.display": true,
          "_ux.display_label": "Implied Sounds",
          "_ux.render_as": "tags"
        },
        "silence_level": {
          "type": "string",
          "_ux.display": true,
          "_ux.display_label": "Silence Level"
        },
        "emotional_intensity": {
          "type": "string",
          "_ux.display": true,
          "_ux.display_label": "Emotional Intensity"
        },
        "warmth": {
          "type": "string",
          "_ux.display": true,
          "_ux.display_label": "Warmth"
        },
        "suggested_music_genres": {
          "type": "array",
          "_ux.display": true,
          "_ux.display_label": "Suggested Genres",
          "_ux.render_as": "tags"
        }
      }
    }
...
**Folder:** `workflows/cc/steps/5_video_generation/` (renamed from
`4_video_prompts`)
...
Add `previous_interactions` to `WorkflowExecutionContext` in
`backend/server/workflow/interaction.py`:

<!--following doesnt look right. it only has access to its own interaction,
which means it has access first interaction of that module and any other
interactions before. read events collection of db, you will understand the
structure, you will see the interaction_requested events have _resolved_inputs,
thats where you will find data you need-->
...
The `media.generate` module reads source image from context for `img2vid`.
...
| `schemas/cc_image_analysis_display_schema.json` | Review display schema |

### Renamed/Modified: `5_video_generation/` (was `4_video_prompts/`)
...
| `9_workflow_summary/prompts/cc_workflow_summary.txt` | Add image analysis section |

### Backend Files

| File | Change |
|------|--------|
| `backend/server/workflow/interaction.py` | Add previous_interactions |
...
### 1. Selected Image local_path

<!--we can update the step 3 to submit image data as needed, lets address this 
when it comes there.-->

The `selected_image_data` from step 3 may not include `local_path`. Options:
- **A)** Step 3 output includes `local_path` in `selected_content`
- **B)** Step 4 module looks up `local_path` from DB using `content_id`
- **C)** Use server URL and let provider resolve

### 2. Sora 2 Implementation

Does OpenAI provider have `img2vid` for Sora 2? May need implementation.

<!--no, you have to implement is. docs:
