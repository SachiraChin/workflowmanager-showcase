...
- [ ] Investigate how `selected_content` is populated
  - Currently: `{content_id, url, metadata_id, prompt_key}`
  - Need: Add `local_path` from database
- [ ] Option A: Query DB in `_find_selected_content()` to get local_path
- [ ] Option B: Include local_path in generations data from sub_action
- [ ] Test: Verify `state.selected_image_data.local_path` is accessible

**2.2 Update media/generate.py**
- [ ] Add `source_image` to module inputs (optional)
- [ ] Pass `source_image` to `display_data` in `get_interaction_request()`
- [ ] Test: Verify interaction request includes source_image

**2.3 Update streaming.py for img2vid**
- [ ] In `execute_sub_action()`, read `_resolved_inputs` from interaction event
- [ ] For `action_type == "img2vid"`, inject `source_image` into params
- [ ] Test: Verify sub-action receives source_image without client round-trip

### Phase 3: WebUI - Video Generation UI

**3.1 Update MediaGeneration component for source_image display**
- [ ] Accept `source_image` from `display_data`
- [ ] Create preview area below prompt controls
- [ ] Display source image (left side of preview)
- [ ] Display generated video (right side, after generation)
...
**4.1 Verify/implement Leonardo img2vid**
- [ ] Check if `img2vid` method exists in Leonardo provider
- [ ] If not, implement based on API docs
- [ ] Test: Generate video from image with Leonardo

### Phase 5: Sora 2 Provider (Optional - can defer)

**5.1 Implement OpenAI Sora 2 img2vid**
- [ ] Read Sora 2 API documentation
- [ ] Add `img2vid` method to OpenAI provider
- [ ] Handle Sora-specific parameters (duration, aspect_ratio)
